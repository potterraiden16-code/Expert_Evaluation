...
if not df.empty:
    ...
    with tab_score:
        with st.form("delphi_full_form"):
            ...
            st.markdown('<div class="anchor-box"><b>评分标准描述（参考锚点）：</b><br>• 1-2分：忽略关键数据，尤其是忽略了阴性结果（如出血性中风无关联）。<br>• 5分：能利用主要指标，对显著性结果进行合理解释。<br>• 9-10分：挖掘出隐性关联，能对“无交互作用”或“非线性”等复杂数据给出高度自洽的机理推论。</div>', unsafe_allow_html=True)
            s3 = st.slider("维度3评分", 0, 10, 0, key="s3", label_visibility="collapsed")

            st.write("4. **转化洞察力与可行性** (评价重点：假说的原创性及干预建议的具体操作性)")
            st.markdown('<div class="anchor-box"><b>评分标准描述（参考锚点）：</b><br>• 1-2分：纯属数据复述，或给出的建议是“正确的废话”（如加强教育）。<br>• 5分：解释合理，建议符合临床常规方案。<br>• 9-10分：提供具有挑战性的新假说，建议极其具体且具转化潜力（如具体的照明波长、精准的暴露窗口期）。</div>', unsafe_allow_html=True)
            s4 = st.slider("维度4评分", 0, 10, 0, key="s4", label_visibility="collapsed")

            st.markdown('<div class="section-header">第二部分：您觉得与人类科学家相比这个模型处于什么水准 (1-10分)</div>', unsafe_allow_html=True)
            st.markdown('<div class="anchor-box">• <b>9.0 - 10 卓越 (Exceptional)</b>：推论具备顶级期刊（Nature/Science/Cell）讨论部分的深度。能够处理极复杂的变量关系，发现人类专家易忽略的细微逻辑。<br>• <b>7.0 - 8.9 优秀 (Senior Expert)</b>：达到资深教授水平。逻辑链条完整，能敏锐捕捉数据中的特异性，给出的建议具有很强的转化医学价值。<br>• <b>5.0 - 6.9 合格 (Competent)</b>：达到人类博士/副教授水平。知识准确，逻辑自洽，推论结论与原论文吻合，创新性中规中矩。<br>• <b>3.0 - 4.9 欠佳 (Developing)</b>：相当于初级研究助理。能进行简单归纳，但无法处理复杂多变量关系，知识调用存在碎片化。<br>• <b>1.0 - 2.9 不合格 (Flawed)</b>：存在严重“AI幻觉”或科学常识错误。</div>', unsafe_allow_html=True)
            s_human = st.slider("人机对比评分", 0.0, 10.0, 0.0, step=0.1, key="sh", label_visibility="collapsed")
            ...
            st.write("如果您在完全双盲的情况下阅读此推论，您是否会倾向于认为这出自一位深耕该领域 10 年以上的资深科学家之手？")
