...
if current_doc_id in reviewed_ids:
    ...
    st.markdown(f'<div class="status-box pending">⏳ 待处理：阅读内容后请切换至“专家评估量表”标签完成评分</div>',
                unsafe_allow_html=True)
...
with tab_score:
    ...
    with st.form("delphi_full_form"):
        ...
        # --- 第二部分 ---
        st.markdown('<div class="section-header">第二部分：与人类科学家相比模型水准 (1-10分)</div>',
                    unsafe_allow_html=True)
        ...
        turing_test = st.radio(
            "如果您在完全双盲（不知作者身份）的情况下阅读此推论，您是否会倾向于认为这出自一位资深科学家之手？",
            ["肯定会", "可能会", "中立", "不太可能", "绝无可能"], horizontal=True)

        submit_button = st.form_submit_button("🚀 提交完整德尔