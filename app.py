...
def get_reviewed_ids():
    ...
    if os.path.exists(results_file):
        try:
            # 这里的读取不再使用缓存，确保每次运行都能拿到文件里的最新 ID
...
with tab_score:
    with st.form("delphi_full_form"):
        ...
        • <b>5.0 - 6.9 合格 (Competent)</b>：达到人类博士/副教授水平。<br>
        • <b>3.0 - 4.9 欠佳 (Developing)</b>：相当于初级研究助理。<br>
        • <b>1.0 - 2.9 不合格 (Flawed)</b>：存在严重“AI幻觉”。
        </div>
        """, unsafe_allow_html=True)
        ...
        consistency = st.selectbox("1. 一致性评价（对比该领域公认科学逻辑）：",
                                   ["高度一致", "基本一致", "存在偏差", "严重违背"])
        ...
        turing_test = st.radio("您是否倾向于认为这出自资深科学家之手？", ["肯定会", "可能会", "中立", "不太可能", "绝无可能"], horizontal=True)
...
if submit_button:
    if not expert_name:
        ...
        try:
            ...
            st.balloons()
            # 强制页面重新运行，此时由于 reviewed_ids 会重新读取，侧边栏和提示文字会立刻同步
            st.rerun() 
