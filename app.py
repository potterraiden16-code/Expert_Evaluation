...
def load_data():
    try:
        # 确保文件名 data_final_v3.xlsx 在同级目录
...
def get_reviewed_ids(expert_name):
    ...
    reviewed = []
    # 扫描目录下所有的csv文件，文件名格式为: {ID}_{timestamp}.csv
...
with st.sidebar:
    ...
    if not df.empty:
        ...
        row = df[df['ID'].astype(str) == current_doc_id].iloc[0]
    ...
    if expert_path:
        ...
        if files:
            expert_all_data = pd.concat([pd.read_csv(f) for f in files])
            csv_buffer = expert_all_data.to_csv(index=False, encoding='utf-8-sig')
            st.download_button(
                label=f"💾 下载您的评价汇总 ({expert_name})",
                data=csv_buffer,
...
# --- 4. 主界面：状态提示 ---
if current_doc_id and current_doc_id in reviewed_ids:
    ...
    with tab_score:
        with st.form("delphi_full_form"):
            ...
            highlights = st.text_area(
                "2. 亮点分析（请说明AI在推演过程中，哪个环节展现了超越人类基准线的洞察力？如认为没有可不答）")
            ...
            st.markdown('<div class="section-header">第四部分：综合标定 (科学图灵测试倾向性提问)</div>',
                        unsafe_allow_html=True)
            st.write(
                "如果您在完全双盲（不知作者身份）的情况下阅读此推论，您是否会倾向于认为这出自一位深耕该领域 10 年以上的资深科学家之手？")
    ...
    if submit_button:
        if not expert_name or expert_name.strip() == "":
            ...
            try:
                ...
                st.balloons()
                st.success(f"✅ 评价提交成功！数据已存入您的个人目录 {expert_dir}")
