...
if submit_button:
    if not expert_name:
        ...
        try:
            ...
            # 成功提示
            st.balloons()
            st.success(f"✅ 文献 {current_doc_id} 评价提交成功！系统已同步更新进度。")
            
            # 【修复关键】强制刷新所有缓存并重启脚本，确保侧边栏 ✅ 状态立即刷新
            st.cache_data.clear()
            st.rerun()
            
        except Exception as e:
            # 【修复关键】增强错误提醒，显示具体原因（如文件被 Excel 打开导致的 Permission Denied）
            st.error(f"❌ 数据保存失败！原因：{e}")
            st.warning("提示：如果是在本地运行，请检查 CSV 文件是否被其他程序（如 Excel）打开并锁定。")
